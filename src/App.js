...
function App() {
    ...
    React.useEffect(() => {
        ...
        fetch(`https://65a01f2d5023b02bfe8b4f52.mockapi.io/collections?page=${page}&limit=3&${category}`)
            .then((res) => res.json())
            .then((json) => {
                setCollections(json)
            })
  ...
  return (
    <div className="App">
      ...
      <ul className="pagination">
          {
              [...Array(5)].map((e,i) => <li onClick={() => setPage(i + 1)} className={page === i+ 1 ? 'active' : ''}>{i+1}</li> )
          }
      </ul>
    </div>
  );
